...
def initialize_graph() -> graphviz.Digraph:
    graph_attr = {"label": str(args.title), "labelloc": "t", "fontsize": "30"}
...
def all_rule_names_with_groups_and_outgoing_edges() -> "list[str]":
    ...
    node_names_with_only_outgoing_edges = (
        rules_by_group_dict.keys() - outgoing_edges_by_node.keys()
    )
...
arg_parser.add_argument(
    "--title", nargs="?", default="", help="Optional title for the graph"
)
