...
impl Rule {
    ...
    pub fn and_then(self,tag_matches: HashMap<String, String>) -> (Vec<Rule>, HashMap<String, Query>){
        ...
        if self.and_then.is_some() {
            for r in self.and_then.unwrap() {
                ...
                for q in &transformed_rule.queries{
                    let query = Query::new(ts_language, q.as_str()).expect(format!("Invalid Query generated, please check {}", q).as_str());
