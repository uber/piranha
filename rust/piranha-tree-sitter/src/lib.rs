...
pub mod piranha {
    ...
    pub fn get_cleanups_for_code_base_new(
        ...
        let mut flag_cleaner = FlagCleaner::new(path_to_code_base, input_language, flag_name, flag_namespace, flag_value);
        flag_cleaner.cleanup();
        flag_cleaner.files.iter().map(|(k, x)| (k.clone(), x.code.clone())).collect()
    }
    ...
    //             if let Ok(replace_node) = get_largest_node_captured_by_query(captures) {
