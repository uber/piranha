...
class RefactorEngine {
    ...
    trueLiteral() { 
        const vanillaLiteral = { // A Boolean Literal is parsed like this by recast
            type: "Literal", 
            value: true, 
            raw: 'true'
        };
    ...
    checkAndAddRedundantFunction(func, name, redundantFunctions) {
        ...
        if (returnIndex < 0) // If returnIndex == -1 when function body is empty
            return false;
        else {
                ...
                if (node.type === 'CallExpression') {
                    if (methodHashMap.has(node.callee.name)) {
                        ...
                        if (nodeArgument.type === "Identifier" && nodeArgument.name === engine.flagname) {       
                            ...
                            if ((flagType === "treated" && engine.behaviour) || (flagType === "control" && !engine.behaviour)) {
    ...
    evalBoolExpressions() {
        ...
        estraverse.replace(this.ast, {
            leave: function(node) { 
                ...
                } else if (node.type === "UnaryExpression" && node.operator === "!" && engine.isPiranhaLiteral(node.argument)) {
    ...
    reduceIfStatements() {
        ...
        estraverse.replace(this.ast, {
            leave: function(node) {
                if ((node.type === "IfStatement" || node.type === "ConditionalExpression") && engine.isPiranhaLiteral(node.test)) {
            ...
            enter: function (node) {
                if (node.type === "VariableDeclaration") {
                    for(var i = 0; i < node.declarations.length; i++) {
                        ...
                        if (declaration.init && engine.isPiranhaLiteral(declaration.init) && typeof(declaration.init.value) === "boolean") {
