...
module.exports = {
    parseProperties: function (properties_json) {
        ...
        if (!("methodProperties" in properties)) {
            ...
            var missingMethod = properties.methodProperties.find(prop => !("methodName", "flagType", "argumentIndex" in prop));
            missingMethod = JSON.stringify(missingMethod);

            if (missingMethod != null) {
                throw new Error(`${missingMethod} in ${properties_json} doesn't have all required keys`);
            }
        }
