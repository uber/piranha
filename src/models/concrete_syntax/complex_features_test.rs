...
use super::parser::{
    ConcreteSyntax, CsPattern, CsElement, CaptureMode, 
    ConstraintKind, ComparisonOp, CsValue
};
...
fn run_test_complex(
    ...
    assert_eq!(matches.len(), expected_matches, 
        "Expected {} matches but got {}", expected_matches, matches.len());
...
fn test_parser_capture_modes() {
    ...
    if let CsPattern::Sequence(elements) = cs.pattern {
        ...
        if let CsElement::Capture { mode: CaptureMode::OnePlus, .. } = elements[0] {
            // OK
        } else {
            panic!("Expected OnePlus capture");
        }
        
        if let CsElement::Capture { mode: CaptureMode::ZeroPlus, .. } = elements[2] {
            // OK  
        } else {
...
fn test_parser_multiple_constraints() {
    let cs = ConcreteSyntax::parse(":[name] where :[name] matches /[A-Z].*/, len(:[name]) > 5").unwrap();
...
fn test_complex_template() {
    run_test_complex(
        ...
        "if (:[cond]) { :[stmts*] }",
        1,
        vec![vec![("cond", "condition"), ("stmts", "statement1; statement2;")]],
        JAVA,
    );
}
