...
use crate::models::matches::{Range, Point, Match};
use super::parser::{ConcreteSyntax, CsPattern, CsElement, CaptureMode, CsConstraint, ConstraintKind, ComparisonOp, CsValue};
    ...
    pub fn get_all_matches(
        ...
        if recursive {
            ...
            for child in node.children(&mut cursor) {
                if let (mut inner_matches, true) = Self::get_all_matches(
                    &child, code_str, cs, recursive, replace_node.clone()
                ) {
        ...
        match &cs.pattern {
            CsPattern::Alternation(alternatives) => {
                ...
                for alternative in alternatives {
                    ...
                    if let Some(result) = Self::match_sequential_siblings(&mut temp_cursor, source_code, &temp_cs) {
                        *cursor = temp_cursor;
                        return Some(result);
                    }
                }
            ...
            CsPattern::Sequence(elements) => {
                Self::match_sequence_pattern(cursor, source_code, elements)
            }
        }
    }
        ...
        if cursor.goto_first_child() {
            ...
            loop {
                ...
                let (mapping, indx) = Self::get_matches_for_element_sequence(
                    &mut tmp_cursor, source_code, elements, true, &parent_node
                );
    ...
    fn get_matches_for_element_sequence(
        ...
        match first_element {
            CsElement::Capture { name, mode } => {
                Self::handle_template_variable_matching_v3(
                    cursor, source_code, top_node, name, *mode, remaining_elements
                )
            }
...
pub fn parse_and_match(
    ...
    Ok(CsInterpreter::get_all_matches(node, code_str, &cs, recursive, replace_node))
}