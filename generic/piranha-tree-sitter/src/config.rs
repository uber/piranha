...
pub struct PiranhaArguments {
    ...
    /// Language for which Piranha cleanup is to be applied
    pub language: String,
...
impl Rule {
    ...
    /// Note this could fail in case when tag matches don't contain mappings for all the holes.
...
impl RuleStore {
    ...
    pub fn get_next(&self, rule: &Rule, tag_matches: &HashMap<String, String>) -> HashMap<String, Vec<Rule>> {
...
pub fn read_rule_graph_from_config(
    ...
    let (mut input_rules, input_edges) = (
        toml::from_str::<Rules>(read_file(&path_to_config.join("rules.toml")).as_str())
            .unwrap(),
        toml::from_str::<Edges>(read_file(&path_to_config.join("edges.toml")).as_str())
            .unwrap(),
    );
