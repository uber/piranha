...
Below we can see the graph for Java pre-built cleanup rules.
...
We use [Graphviz](https://graphviz.org/) to generate a graph image from a .dot file generated by `visualize_rules_graph.py`.
Please follow the instructions to install Graphviz at <https://graphviz.org/download/>.
Moreover, the script also needs the `toml` python package.
...
To generate the SVG used in this README and assuming a Python venv:

```bash
pip install toml
python visualize_rules_graph.py ./java_prebuilt_rules.dot src/cleanup_rules/java --title "Java pre-built cleanup rules"
dot -Tsvg java_prebuilt_rules.dot -o java_prebuilt_rules.svg
```
...
dot -Tsvg java-ff_system1.dot -o java-ff_system1.svg
```
