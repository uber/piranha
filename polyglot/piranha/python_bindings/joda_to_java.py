from polyglot_piranha import Rule, Edge, Constraint, RuleGraph
from query_utils import java_import_declaration_type, filter_eq, java_type_identifier, filter_not_eq, java_method_invocation, java_file, query, rename_tags, transform_tags

path_to_piranha_bin = "/Users/ketkara/repositories/open-source/test_piranha_config"
path_to_piranha_config = "/Users/ketkara/repositories/open-source/uber_piranha/polyglot/piranha/test-resources/java/joda_to_java/only_expressions_usage/input"
path_to_target = "/Users/ketkara/repositories/open-source/test_piranha_config"




cns = Constraint(matcher=query(java_file, []),
                 queries=[query(transform_tags(java_import_declaration_type, lambda i: 'c_' + i),
                                [filter_eq('c_type_qualifier', 'org.joda.time'), filter_not_eq('c_type_name', 'Duration')]),
                          query(java_type_identifier, [filter_eq("type_identifier", "Duration")])])

update_duration_import = Rule('update_duration_import',
                              query=query(java_import_declaration_type, [
                                  filter_eq("imported_type", "org.joda.time.Duration")]),
                              replace='java.time.Duration',
                              replace_node='imported_type',
                              constraints=[cns])


update_standard_hours = Rule("update_standard_hours",
                             query=query(java_method_invocation, [
                                         filter_eq("receiver", "@type_name"), filter_eq("name", "standardHours")]),
                             replace="ofHours",
                             replace_node="method_invocation.name",
                             holes=["type_name"])


rule_graph = RuleGraph(["java"])
rule_graph.add_rule(update_duration_import)
rule_graph.add_rule(update_standard_hours)

rule_graph.add_edge(Edge(update_duration_import, [
                    update_standard_hours], "File"))

rules, edges, piranha_arguments = rule_graph.as_tomls()

output = rule_graph.apply(path_to_piranha_bin,
                          path_to_target,
                          path_to_piranha_config)

print(output)
print(rules)
print("------")
print(edges)
print("-------")
print(piranha_arguments)

#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#

